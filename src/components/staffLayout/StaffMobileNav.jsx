...
import { STAFF_MOBILE_NAV_ITEMS } from "./staffNavConfig";
...
export default function StaffMobileNav() {
  return (
    <nav className="lg:hidden fixed bottom-0 inset-x-0 z-40 pb-[max(env(safe-area-inset-bottom),0.5rem)] px-3 pt-1.5">
      <div className="relative flex items-stretch justify-between gap-1 rounded-[26px] bg-gradient-royal shadow-lift ring-1 ring-white/10 px-1.5 py-1.5 backdrop-blur">
        {/* top hairline glow */}
        ...
        {STAFF_MOBILE_NAV_ITEMS.map(({ to, label, icon: Icon, end }) => (
          <NavLink
            key={to}
            to={to}
            end={end}
            className="relative flex-1 flex flex-col items-center justify-center gap-1 py-1.5 rounded-2xl text-[10px] font-medium transition-all min-h-[44px]"
          >
            {({ isActive }) => (
              <>
                <span
